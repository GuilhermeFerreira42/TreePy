...
def processar_diretorio():
    """
    Função para abrir um diretório, processar os arquivos Python e exibir as árvores de cada um.
    """
    ...
    if diretorio:
        ...
        for filename in os.listdir(diretorio):
            ...
            if os.path.isfile(filepath) and filepath.endswith(('.py', '.txt', '.html')):
                with open(filepath, 'r', encoding='utf-8') as file:
                    code = file.read()
                    root = parse_code(code)
                    tree_output += f"\nÁrvore para o arquivo: {filename}\n"
...
# Botão para abrir o diretório e processar os arquivos
botao_abrir = ttk.Button(root, text="Abrir Diretório", command=processar_diretorio)
